...
class Petstoreflownew extends Simulation {
	...
	val scn = scenario("Petstoreflownew")
		...
		.exec(http("Logout")
			...
			.check(status.is(404)),
            http("request_90")
			.get("/applicationPetstore/javax.faces.resource/fontawesoming e/4.7.0/fonts/fontawesome-webfont.ttf?v=4.7.0")
			.headers(headers_40)
			.check(status.is(404))))
	...
	setUp(
		scn.inject(constantConcurrentUsers(15) during(900.second ))).maxDuration(15.minutes)
			//atOnceUsers(1),
